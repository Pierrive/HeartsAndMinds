...
if (!_is_init) then {
    private _ratio = (switch _type do {
        case "Hill" : {random 1};
        case "NameLocal" : {random 2};
        case "NameVillage" : {random 3};
        case "NameCity" : {random 4};
...
if !(_data_units isEqualTo []) then {
    ...
    if (_has_en) then {
        for "_i" from 1 to (round (_p_mil_group_ratio * (1 + random _max_number_group))) do {
			[_city, _radius, 1 + round random [0, 2, 4] , random 1] call btc_fnc_mil_create_group;
		};
		if !(_has_ho) then {
			([_city, _radius] call btc_fnc_city_findPos) params ["_rpos", "_pos_iswater"];
			private _random = random 1;
			switch (true) do {
				case (_random <= 0.3) : {
					private _statics = btc_type_mg;
					[[(_rpos select 0) + (round random 100), (_rpos select 1) + (round random 100), 0], _statics, (round random 360)] call btc_fnc_mil_create_static;
				};
				case (_random > 0.3 && _random <= 0.79) : {
...
if (_has_ho && {!(_city getVariable ["ho_units_spawned", false])}) then {
    ...
    [_pos, 20, 4 + round (_p_mil_group_ratio * random 6), 1.1] call btc_fnc_mil_create_group;
