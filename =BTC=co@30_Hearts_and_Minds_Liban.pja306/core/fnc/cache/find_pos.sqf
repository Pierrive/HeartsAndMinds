...
private _useful = _city_all select {_x getVariable ["occupied", false] && {!(_x getVariable ["type", ""] in ["NameLocal", "NameCity", "NameMarine"])}};
...
if (_city getVariable ["type", ""] in ["NameLocal", "NameCity", "NameMarine"]) exitWith {
    [] call btc_fnc_cache_find_pos;
};
