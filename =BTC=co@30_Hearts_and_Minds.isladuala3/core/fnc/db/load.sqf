...
//nbSpawnVeh
_nbVeh = profileNamespace getVariable "nbSpawnVeh";
_nbVehMax = profileNamespace getVariable "nbSpawnVehMax";
ambianceBase setVariable ["btc_nb_veh_create", _nbVeh, true];
ambianceBase setVariable ["btc_max_nb_veh_create", _nbVehMax, true];
...
diag_log format ["%1", _markers_properties];
{
    _x params ["_markerText", "_markerPos", "_markerColor", "_markerType", "_markerSize", "_markerAlpha", "_markerBrush", "_markerDir", "_markerShape","_getMarkerTyp"];

    private _marker = createMarker [format ["_USER_DEFINED #0/%1/1", _forEachindex], _markerPos];
    _marker setMarkerText _markerText;
    _marker setMarkerColor _markerColor;
    _marker setMarkerType _markerType;
	if (markerType _marker == "") then {_marker setMarkerType _getMarkerTyp};
	if (markerType _marker == "") then {_marker setMarkerType "hd_dot"};
