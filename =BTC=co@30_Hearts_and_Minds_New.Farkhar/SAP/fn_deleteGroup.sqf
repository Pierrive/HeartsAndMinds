...
if (count _ArrayAllGrps > 0) then {
	{
		...
		{
			if ((_x distance _leaderGrp) > 2000) then {
				if !(_leaderGrp isEqualTo _vehicleGrp) then {{ _vehicleGrp deleteVehicleCrew _x; sleep 0.1 } forEach crew _vehicleGrp; deleteVehicle _vehicleGrp};
				{deleteVehicle _x; sleep 0.1;} forEach units (group _leaderGrp);
