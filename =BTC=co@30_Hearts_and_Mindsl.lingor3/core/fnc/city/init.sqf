...
private _cities = ["NameVillage", "NameCity", "NameCityCapital", "NameLocal", "Hill", "Airport","StrongpointArea"];
...
for "_i" from 0 to (count _locations - 1) do {
    ...
    if (_type in _cities) then {
        ...
        private _radius_x = (getNumber(_current >> "RadiusA")) + 200;
        private _radius_y = (getNumber(_current >> "RadiusB")) + 200;

        if (btc_city_blacklist find _name >= 0) exitWith {};

        //if you want a safe area
        if ((getMarkerPos "BaseBLU") inArea [_position, 500, 500, 0, false]) exitWith {};
