if (isServer) then {
	...
	// On body delivery:
	[{params["_killer", "_nextofkin", "_body"]; btc_rep_bonus_civ_hh call btc_fnc_rep_change;}] call GR_fnc_addDeliverBodyEventHandler; // args [_killer, _nextofkin, _body]
	//[yourCustomEvent_OnDeliverBody] call GR_fnc_removeDeliverBodyEventHandler;

	// On successful concealment of a death:
	[{params["_killer", "_nextofkin", "_body"]; btc_rep_bonus_civ_hh call btc_fnc_rep_change;}] call GR_fnc_addConcealDeathEventHandler; // args [_killer, _nextofkin, _grave]
