...
if ((_randomEvent > 5) && (_randomEvent < 15)) then {
	...
	private _spawnposAir = [_player, 2000, 3000, 2, 0, 0, 0, [_safeZone], []] call BIS_fnc_findSafePos;
	private _grp = [vlr_class_units, 1, _spawnposAir] call fnc_spawnGrpUnits;
	private _vehicle = [_grp, _spawnposAir, vlr_class_TransportAir] call fnc_createVeh;
	waitUntil {missionNamespace getVariable "CheckVehReady"};
	missionNamespace setVariable ["CheckVehReady", false, true];
	[_grp, _vehicle, _player, _spawnposAir, _safeZone] spawn fnc_addWPTransport;
};
...
if ((_randomEvent > 36) && (_randomEvent < 63)) then {
	...
	private _grp = [vlr_class_units, 5] call fnc_spawnGrpUnits;
...
if ((_randomEvent > 63) && (_randomEvent < 73)) then {
	...
	if ([_spawnpos] call fnc_checkRoadNear) then {
		private _grp = [vlr_class_units, 1, _spawnpos] call fnc_spawnGrpUnits;
		...
		[_spawnpos,_player,_safeZone,80] call fnc_randomFootPlayer
	};
};
...
if ((_randomEvent > 73) && (_randomEvent <= 100)) then {
	...
	private _grp = [vlr_class_units, 4] call fnc_spawnGrpUnits;
