...
private _markers_properties = _player_markers apply {
    [markerText _x, markerPos _x, markerColor _x, markerType _x, markerSize _x, markerAlpha _x, markerBrush _x, markerDir _x, markerShape _x, getMarkerType _x]
};
profileNamespace setVariable [format ["btc_hm_%1_markers", _name], _markers_properties];
diag_log format ["%1", _markers_properties];

//nbSpawn
_nbVeh = ambianceBase getVariable "btc_nb_veh_create";
_nbVehMax = ambianceBase getVariable "btc_max_nb_veh_create";
profileNamespace setVariable ["nbSpawnVeh", _nbVeh];
profileNamespace setVariable ["nbSpawnVehMax", _nbVehMax];
