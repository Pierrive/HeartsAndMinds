...
private _allHouse = _ArrayHouseMiscel + _ArrayHouseVanilla;
...
Amb_Alive_Sound = [{
	...
	_ArrayPetsJungle = ["elephant", "jaguar", "lion", "panda", "perroquet", "singe"];
	private _soundPetsJungle = selectRandom _ArrayPetsJungle;
	...
	if ((count _humanPlayers > 0) && !(_player inArea _safeZone)) then {
		...
		private _PosFar = [_player, 100, 250, 0, 0, 0, 0, [_safeZone], []] call BIS_fnc_findSafePos;
		private _PosNear = [_player, 10, 100, 0, 0, 0, 0, [_safeZone], []] call BIS_fnc_findSafePos;
		...
		private _allTree = nearestTerrainObjects [_PosNear, ["TREE", "SMALL TREE"], 20, true];
		private _allMen = ASLToAGL getPosASL _player nearEntities [["Man"], 300];		
		...
		if (count _allBuildings > 0) then {
			...
			if (count _allMen > 0) then {
				{
					if (side _x == btc_enemy_side) then {
						if (behaviour _x isEqualTo "COMBAT") then {_allGroupFightArea pushback _x};
					};
				} foreach _allMen;
			};
			...
			if (count _allGroupFightArea < 4) then {
				...
				/*
				//Random Pets Jungle Song
				if ((count _allTree > 0) && ((random 1) > 0.59)) then {
					_randomPosSound = selectRandom _allTree;
					_posMusic = getPosASL _randomPosSound;
					_Sound = format ["\core\sounds\animaux\%1.ogg", _soundPetsJungle];
					playSound3D [getMissionPath _Sound, _randomBuildMus, false, _posMusic, 1, 1, 300];
				};
				*/	
...
}, ((round (random 100)) + 100), [_safezone, _allHouse, _ArrayReligionCatho]] call CBA_fnc_addPerFrameHandler;
